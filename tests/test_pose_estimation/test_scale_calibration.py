...
def test_video_scale_recovers_known_mpp_thigh_only():
    ...
    mpp, info = compute_per_frame_scale_mpp(
        lm, image_width=W, image_height=H, thigh_length_m=0.43,
    )
...
def test_video_scale_median_across_thigh_and_shank():
    ...
    mpp, info = compute_per_frame_scale_mpp(
        ...
        thigh_length_m=0.43, shank_length_m=0.47,
    )
    assert mpp is not None
    expected_thigh = 0.43 / 200.0
    expected_shank = 0.47 / 220.0
...
def test_video_scale_rejects_foreshortened_frames():
    ...
    mpp, info = compute_per_frame_scale_mpp(
        lm, image_width=W, image_height=H,
        thigh_length_m=0.43, projection_percentile=95.0,
    )
    assert mpp is not None
    # True in-plane scale: 0.43 / 200 ≈ 2.15e-3.  Foreshortened-biased scale
    # would be 0.43 / 50 = 8.6e-3 — 4× too large.
    truth = 0.43 / 200.0
...
def test_video_scale_skips_low_visibility_landmarks():
    ...
    np.testing.assert_allclose(mpp, 0.43 / 200.0, rtol=1e-6)
...
def test_video_scale_no_visible_segments_returns_none():
    ...
    mpp, info = compute_per_frame_scale_mpp(
        lm, image_width=1920, image_height=1080, thigh_length_m=0.43,
    )
    assert mpp is None
    assert "warning" in info
...
def test_calibrate_uses_multi_segment_when_lengths_provided():
    ...
    # Imogen-like proportions: thigh = 200 px, shank = 220 px.
    ankle_y_norm = 0.95
    ...
    calibrated = calibrate_landmarks_to_world(
        lm_2d, lm_3d, height_m=1.78,
        image_width=W, image_height=H,
        thigh_length_m=0.43, shank_length_m=0.47,
    )
    nose_y = float(calibrated[:, 0, 1].mean())
    # 940 px × (0.43/200 m/px) = 2.021 m ankle-to-nose.
