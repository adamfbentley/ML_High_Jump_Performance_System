...
class TestSegmentEnforcement:
    def test_thigh_length_matches_anthropometry(self):
        ...
        lm[:, 23, :3] = [0.0, 1.0, 0.0]
        lm[:, 25, :3] = [0.0, 0.5, 0.0]  # 0.5 m apart (too long for 0.245 * 1.75 = 0.456)
    ...
    def test_weight_zero_no_change(self):
        lm = _make_landmarks(50)
        result = enforce_segment_lengths(lm, 1.75, weight=0.0)
        np.testing.assert_array_equal(result, lm)
    ...
    def test_direction_preserved(self):
        ...
        result = enforce_segment_lengths(lm, 1.75, weight=1.0)
...
class TestFullPipeline:
    def test_default_config_runs(self):
        ...
        config = PostProcessorConfig(height_m=1.75)
