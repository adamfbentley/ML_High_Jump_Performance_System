...
def test_tensor_ordering_first_nine_unchanged():
    ...
    assert abs(t[4] - 65.0) < 1e-5   # plant_angle_deg
...
def test_new_fields_at_positions_9_to_13():
    """Positions 9–13 in tensor correspond to the five new Athlete A-priority fields."""
    tp = TechniqueParameters(
        approach_speed_mps=7.0,
        curve_radius_m=9.5,
        penultimate_step_length_cm=170.0,
...
def test_com_height_full_extension():
    ...
    h = _estimate_takeoff_com_height(1.75, 180.0, 180.0)
    expected = 1.75 * 0.55  # 0.979 m
...
def test_com_height_decreases_with_flexion():
    ...
    h_full = _estimate_takeoff_com_height(1.75, 180.0, 180.0)
    h_bent = _estimate_takeoff_com_height(1.75, 160.0, 170.0)
...
def test_vertical_velocity_increases_with_approach_speed():
    ...
    v_slow = _impulse_model_vertical_velocity(5.0, 65.0, 120.0, 3.0, 50.0, 65.0)
    v_fast = _impulse_model_vertical_velocity(8.0, 65.0, 120.0, 3.0, 50.0, 65.0)
...
def test_vertical_velocity_increases_with_plant_angle():
    ...
    v_shallow = _impulse_model_vertical_velocity(7.0, 55.0, 120.0, 3.0, 50.0, 65.0)
    v_steep = _impulse_model_vertical_velocity(7.0, 75.0, 120.0, 3.0, 50.0, 65.0)
...
def test_predicted_height_physically_plausible():
    ...
    result = predict_bar_clearance(params, ATHLETE_A_MASS, ATHLETE_A_HEIGHT)
...
def test_sensitivity_returns_all_params():
    ...
    params = _default_params()
    sens = compute_sensitivity(params.to_tensor(), ATHLETE_A_MASS, ATHLETE_A_HEIGHT)
    assert len(sens) == 14
    for name in [
...
def test_differentiable_model_matches_analytical():
    ...
    params = _default_params()
    analytical = predict_bar_clearance(params, ATHLETE_A_MASS, ATHLETE_A_HEIGHT)

    diff_h = _evaluate_height_differentiable(
        params.to_tensor(), ATHLETE_A_MASS, ATHLETE_A_HEIGHT,
    ).item()
...
def test_differentiable_model_has_gradients():
    ...
    h = _evaluate_height_differentiable(params, ATHLETE_A_MASS, ATHLETE_A_HEIGHT)
    h.backward()
...
def test_optimize_technique_improves_height():
    ...
    result = optimize_technique(
        params, ATHLETE_A_MASS, ATHLETE_A_HEIGHT, n_iterations=50,
    )
...
def test_what_if_speed_increase():
    ...
    result = what_if_scenario(
        params, ATHLETE_A_MASS, ATHLETE_A_HEIGHT,
...
def test_coaching_cues_ordered_by_impact():
    ...
    sens = compute_sensitivity(current.to_tensor(), ATHLETE_A_MASS, ATHLETE_A_HEIGHT)
