...
Covers the updated MOVEMENT_RELEVANCE ordering reflecting Athlete A's
...
# ── Ranking order (Athlete A priority) ──────────────────────────────────────
...
def test_drop_jump_above_cmj():
    """Box drop jump should rank higher than CMJ per Athlete A."""
    assert (
        MOVEMENT_RELEVANCE[MovementType.DROP_JUMP]
