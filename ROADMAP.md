...
- Loads `.npz` cache, applies a Phase-9a scale guardrail (rejects clips with
  ...
  `data/models/personal/athlete_a_finetuned.pth`. Has `--dry-run`.
...
- `scripts/analyze_jump_video.py` adds `--thigh` / `--shank` CLI flags
  (defaults 0.45 / 0.45 — Athlete A's measured values).
...
- Re-ran `scripts/analyze_jump_video.py` over all 45 private high-jump videos
  with `--save-samples data/results/samples --thigh 0.45 --shank 0.45`.
...
- `--capture-mode stationary --thigh 0.45 --shank 0.45` on production run;
