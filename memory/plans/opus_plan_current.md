...
1. **Surface-anatomical vs. joint-center mismatch.** Athlete A's reported
   thigh = 0.45 m is almost certainly a tape-measure greater-trochanter to
   ...
   longer than 0.45 m and our derived mpp is therefore biased *down* by a
...
3. **The 95th percentile assumes there are in-plane frames.** If the J-curve
   ...
   underestimates true segment length. Athlete A's J-curve does pass through
...
4. **Bar-height ground truth.** Construct a synthetic case with the bar
   at scene Y = 1.75 m, run a trivial detector that returns the bar's
...
3. Reprocess all 45 clips with `--scene-anchor on --thigh 0.45 --shank 0.45
